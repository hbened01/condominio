...
awk '/"CONJ. RES."/' $path"copiacondc.txt" > $path"copiacondc2.txt"
sed 's/",*/;/g' $path"copiacondc2.txt" > $path"copiacondc3.txt"
sed 's/,;/;/g' $path"copiacondc3.txt" > $path"copiacondc4.txt"
...
do
	...
	if [ $flag4 = 0 ] 
		...
		if [ $rid != 0 ] && [ $flag4 = 0 ]
			then
			echo "FACTURAS YA FUERON CARGADAS ANTERIORMENTE"
			exit $?
		else
			flag4=1
		fi
...
done < $path"copiacondc4.txt"

echo "FACTURAS CARGADAS CORRECTAMENTE"